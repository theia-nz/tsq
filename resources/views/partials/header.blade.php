<header x-data="{ sideMenu: false }">
	<div class="container mx-auto px-[15px] py-[30px] flex items-center justify-between">
		...
		<i class="fa-solid fa-bars w-[40px] h-[40px] cursor-pointer transition-all hover:text-secondary-grey"
	...
	<div class="flex flex-row-reverse absolute inset-0" x-cloak x-show="sideMenu">
		<div class="cursor-pointer absolute inset-0" x-show="sideMenu" x-on:click="sideMenu = false"
			...
			<div class="bg-quaternary-grey bg-opacity-50 absolute inset-0"></div>
		</div>
		<div class="bg-quaternary-grey w-full max-w-xl pt-[160px] px-[60px]" x-show="sideMenu"
			...
			<div class="flex flex-col gap-y-[20px] relative">
				<i class="fa-solid fa-xmark text-white w-[50px] h-[50px] cursor-pointer transition-all hover:text-secondary-grey absolute top-[-90px] right-0"
					x-on:click.stop="sideMenu = false"></i>
				@foreach($menu->getSiblings()->where('published', 1) as $menuIndex => $menuItem)
				<div
					class="font-primary font-thin uppercase text-[36px] text-white transition-all hover:text-secondary-grey relative">
					...
					<div class="flex flex-col gap-y-[20px] pt-[20px] pl-[40px]">
