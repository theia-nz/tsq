<header class="bg-white bg-opacity-95 w-full fixed z-40 top-0" x-data="{ sideMenu: false }">
	<div class="container mx-auto px-4 py-[5px] flex items-center justify-between">
		<div class="relative mr-[15px]">
			<a class="absolute inset-0" href="{{ config('app.url') }}"></a>
			<img class="max-h-[40px]" src="{{ $setting->where('key', 'company_logo')->first()->image('company_logo') }}"
				alt="">
		</div>
